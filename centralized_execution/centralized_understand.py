...
logging.basicConfig(
    filename=config.log_path + "centralized_understand.log", 
    format=config.log_format ,
    level=config.log_level,
    datefmt=config.log_datefmt)

def performUnderstandAllVul(model_path, filename):
    x_train, y_train, x_test, y_test, x_val, y_val = centralized.getAllData(config.understand_csv_files)
...
def performUnderstandAllScore(model_path, filename):
    x_train, y_train, x_test, y_test, x_val, y_val = centralized.getAllData(config.understand_csv_files)
...
def performUnderstandCFSVul(model_path, filename):
    x_train, y_train, x_test, y_test, x_val, y_val = centralized.getAllData(config.understand_cfs_vul_files)
    model = centralized.getVulModel(model_path)
    initial_history = centralized.trainVulModel(model, x_train, y_train, x_val, y_val)
    centralized.predictAndEvaluateVulModel(model, initial_history, filename, x_train, y_train, x_test, y_test, x_val, y_val)

def performUnderstandCFSScore(model_path, filename):
    x_train, y_train, x_test, y_test, x_val, y_val = centralized.getAllData(config.understand_cfs_score_files)
    model = centralized.getScoreModel(model_path)
    estimator = centralized.getScoreEstimator(model, x_train, y_train)
    initial_history = centralized.trainScoreModel(estimator, x_train, y_train, x_val, y_val)
    centralized.predictAndEvaluateScoreModel(estimator, initial_history, filename, x_train, y_train, x_test, y_test, x_val, y_val)
